...
import L, { latLng } from 'leaflet'; //const URL_TRACKS = 'https://ergast.com/api/f1/2021/circuits.json?limit=100';
...
let circles = [];
let layerCircles = L.layerGroup().addTo(map); // Dessiner un cercle
...
function drawCircle(lat, long, accidents, name) {
  ...
  circle.bindPopup(L.popup().setContent(name)).on('mouseover', function () {
    this.openPopup();
